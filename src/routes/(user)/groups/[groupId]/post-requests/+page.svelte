...
<div class="flex relative h-screen bg-orange-50">
	<!-- Main Content -->
	<div class="flex-1 xl:px-12 p-4 overflow-y-auto">
		<div class="p-4 rounded-lg text-black">
			<div class="text-center py-4">
				<h1
					class="text-2xl font-bold mb-2 text-black hover:text-gray-700"
				>
			...
			<div class="bg-orange-300 p-4 rounded-lg shadow-lg">
				{#each data.postRequests as post}
					...
					>
						<div class="mb-8">
							<p
								class="mb-4 text-black font-bold hover:text-gray-700"
							>
								User {post.username} has added a post with the following
								tags and images on dated {post.createdat}.
							</p>

							<p
								class="mb-4 text-purple-800 font-bold hover:text-gray-700"
							>
								{post.caption}
							</p>
							...
							<div class="tags flex flex-wrap gap-2 mb-4">
								{#each post.tagnames as tag, index}
									<span
										class="tag bg-orange-300 rounded px-2 py-1"
									>
										{tag}
										<button
											on:click={() =>
												removeTag(post, tag)}
											class="ml-1 text-black hover:text-white"
											>&times;</button
										>
									</span>
								{/each}
