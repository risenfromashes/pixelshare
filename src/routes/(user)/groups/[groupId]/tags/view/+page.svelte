...
<div class="p-4 xl:p-10">
  ...
  <div class="flex flex-row flex-wrap gap-x-2 gap-y-4 px-24">
    {#if data.error}
      <div
    ...
    {:else}
      {#each data.tags as tag (tag.id)}
        <div
          class="w-full tag-item flex items-center bg-orange-100 border-2 border-orange-200 p-4 rounded-lg shadow-lg hover:bg-gray-50 transition-colors duration-200 ease-in-out"
        >
          <!-- Tag Image -->
          ...
          <div class="flex-1">
            ...
            <div class="flex justify-between items-center text-sm mt-2">
              <time>{new Date(tag.createdAt).toLocaleDateString()}</time>
              <span
