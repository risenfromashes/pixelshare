<script lang="ts">
  ...
  function handleAction(event: Event, approvalStatus: boolean) {
...
<div class="requests-panel p-4 rounded-lg px-24">
  <!-- Group Name -->
  <h1 class="text-3xl font-bold text-center mb-8">Current Admins</h1>
  <div class="flex flex-col space-y-4">
    ...
    {:else if !data.adminMembers || data.adminMembers.length === 0}
      <p class="text-center">No admins yet.</p>
    {:else}
      {#if success}
        <div
      ...
      <div class="grid grid-cols-2 gap-4">
        {#each data.adminMembers as member}
          <div
            class="flex items-center space-x-4 bg-orange-100 p-4 rounded-lg border border-orange-400 shadow-md hover:bg-orange-200"
          >
            <!-- Member Picture -->
            <img
